...
def main() -> None:
    ...
    dates = pd.to_datetime("2024-01-01") + pd.to_timedelta(rng.integers(0, 540, N), unit="D")
    ...
    df = pd.DataFrame(
        {
            ...
            "desk": rng.choice([f"desk_{i:03d}" for i in range(220)], N),  # high-cardinality
            "notional": notional,                                # outliers
            "pnl": pnl,
            "fee_bps": rng.normal(2.5, 0.4, N),
            "comment": rng.choice(["", "", "", "check", "manual adj"], N),  # mostly empty
